...
#include <sepanta_msgs/objectsposition.h>
#include <Eigen/Dense>
#include <stdio.h>
#include <iostream>
#include <math.h>
...
int Global_end()
{
    gClinet->setMotor_rightShoulderYaw(gClinet->getMotor_rightShoulderYaw().init,300);
    gClinet->setMotor_rightShoulderPitch(gClinet->getMotor_rightShoulderPitch().init,300);
    gClinet->setMotor_rightShoulderRoll(gClinet->getMotor_rightShoulderRoll().init,300);
    gClinet->setMotor_rightElbowPitch(gClinet->getMotor_rightElbowPitch().init,300);
    gClinet->setMotor_rightElbowRoll(gClinet->getMotor_rightElbowRoll().init,300);
    gClinet->setMotor_rightWristPitch(gClinet->getMotor_rightWristPitch().init,300);
    gClinet->setMotor_rightWristRoll(gClinet->getMotor_rightWristRoll().init,300);

    gClinet->setMotor_leftShoulderYaw(gClinet->getMotor_leftShoulderYaw().init,300);
    gClinet->setMotor_leftShoulderPitch(gClinet->getMotor_leftShoulderPitch().init,300);
    gClinet->setMotor_leftShoulderRoll(gClinet->getMotor_leftShoulderRoll().init,300);
    gClinet->setMotor_leftElbowPitch(gClinet->getMotor_leftElbowPitch().init,300);
    gClinet->setMotor_leftElbowRoll(gClinet->getMotor_leftElbowRoll().init,300);
    gClinet->setMotor_leftWristPitch(gClinet->getMotor_leftWristPitch().init,300);
    gClinet->setMotor_leftWristRoll(gClinet->getMotor_leftWristRoll().init,300);
...
int grip_right_check(float x,float y,float z,float a,float b,float g)
{
    ...
    double QsyH=-4*pi/180;
    double QszL=-65*pi/180;
    double QszH=65*pi/180;
    double QeyL=-90*pi/180;
    double QeyH=0*pi/180;
    double QwzL=-180*pi/180;
    double QwzH=180*pi/180;
    double QwyL=-45*pi/180;
    double QwyH=45*pi/180;
    double QwxL=-38*pi/180;
    double QwxH=10*pi/180;
...
int grip_process_right_check(double xend,double yend,double zend)
{
    ...
    double RobotGo=0.45; //if Robot moves forward
    ...
    if(grip_right_check(0.4,-0.43,-0.2,0,-90,0)==0)
    {
        std::cout<<std::endl<<"problem 1"<<std::endl;

        return 0;
    }
    else if(grip_right_check(X-a,Y-b-0.05,Z,A,B,G)==0)
    {
        std::cout<<std::endl<<"problem 2"<<std::endl;
        return 0;
    }

    else if(grip_right_check(X,Y,Z,A,B,G)==0)
    {
        std::cout<<std::endl<<"problem 3"<<std::endl;
        return 0;
    }

    else if(grip_right_check(X-0.05,Y,Z+0.1,A,B,G)==0)
    {
        std::cout<<std::endl<<"problem 4"<<std::endl;
        return 0;
    }

    else if(grip_right_check(0.4,-0.43,-0.2,0,-90,0)==0)
    {
...
int grip_process_right(double xend,double yend,double zend)
{
    ...
    std::cout<<std::endl<<"Xg is = "<<Xg<<std::endl;
    std::cout<<std::endl<<"Yg is = "<<Yg<<std::endl;
    std::cout<<std::endl<<"Z is = "<<Z<<std::endl;
    ...
    grip_right(0.4,-0.43,-0.23,0,-90,0);
    ...
    upperbodyclient::motor_data PW = gClinet->getMotor_waist();


    double Qw=1830;
    int speedw=((fabs(Qw-PW.position)*fabs(AsYr)/6)/(Time))/0.01;
    gClinet->setMotor_waist(Qw,speedw);


    // Right hand

    double Qs_Ysend=2338;
    int speeds_Y=((fabs(Qs_Ysend-PRH1.position)*fabs(AsYr)/6)/(Time))/0.01;
    gClinet->setMotor_rightShoulderYaw(Qs_Ysend,speeds_Y);

    double Qs_Xsend=3302;
    int speeds_X=((fabs(Qs_Xsend-PRH2.position)*fabs(AsXr)/6)/(Time))/0.01;
    gClinet->setMotor_rightShoulderPitch(Qs_Xsend,speeds_X);

    double Qs_Zsend=1050;
    int speeds_Z=((fabs(Qs_Zsend-PRH3.position)*fabs(AsZr)/6)/(Time))/0.01;
    gClinet->setMotor_rightShoulderRoll(Qs_Zsend,speeds_Z);

    double Qe_Ysend=2162;
    int speede_Y=((fabs(Qe_Ysend-PRH4.position)*fabs(AeYr)/6)/(Time))/0.01;
    gClinet->setMotor_rightElbowPitch(Qe_Ysend,speede_Y);

    double Qw_Zsend=2959;
    int speedw_Z=((fabs(Qw_Zsend-PRH5.position)*fabs(AwZr)/6)/(Time))/0.01;
    gClinet->setMotor_rightElbowRoll(Qw_Zsend,speedw_Z);

    double Qw_Ysend=2280;
    int speedw_Y=((fabs(Qw_Ysend-PRH6.position)*fabs(AwYr)/6)/(Time))/0.01;
    gClinet->setMotor_rightWristPitch(Qw_Ysend,speedw_Y);

    double Qw_Xsend=1607;
    int speedw_X=((fabs(Qw_Xsend-PRH7.position)*fabs(AwXr)/6)/(Time))/0.01;
    gClinet->setMotor_rightWristRoll(Qw_Xsend,speedw_X);
...
int grip_left(float x,float y,float z,float a,float b,float g)
{
    ...
    std::cout << "Left hand run"<<std::endl;
    ...
    if (Flag==0)
    {
        std::cout << "There is not any answer(Left)."<<std::endl;
    }
    // std::cout<<"desired angles are="<<Qs_Yrad<<std::endl<<Qs_Xrad<<std::endl<< Qs_Zrad<<std::endl<< Qe_Yrad<<std::endl<< Qw_Zrad<<std::endl<< Qw_Yrad<<std::endl<<Qw_Xrad<<std::endl <<std::endl ;
...
bool callback_grip(sepanta_msgs::grip::Request& request, sepanta_msgs::grip::Response& response)
{
...
bool callback_amg(sepanta_msgs::grip::Request& request, sepanta_msgs::grip::Response& response)
{
    ...
    while (ros::ok() && appExit == false )
    {
        if ( start_grip )
        {
            ...
            if ( right_left == "right")
                ...
                if ( right_left == "right")
                {
                    result = grip_process_right_check(end_position.x,end_position.y,end_position.z);
                    if(result==1)
                    {
...
int Carrying()
{
    ...
    double Qs_Ysend=2240;
    int speeds_Y=((fabs(Qs_Ysend-PRH1.position)*fabs(AsYr)/6)/(Time))/0.01;
    gClinet->setMotor_rightShoulderYaw(Qs_Ysend,speeds_Y);

    double Qs_Xsend=3298;
    int speeds_X=((fabs(Qs_Xsend-PRH2.position)*fabs(AsXr)/6)/(Time))/0.01;
    gClinet->setMotor_rightShoulderPitch(Qs_Xsend,speeds_X);

    double Qs_Zsend=1057;
    int speeds_Z=((fabs(Qs_Zsend-PRH3.position)*fabs(AsZr)/6)/(Time))/0.01;
    gClinet->setMotor_rightShoulderRoll(Qs_Zsend,speeds_Z);

    double Qe_Ysend=1739;
    int speede_Y=((fabs(Qe_Ysend-PRH4.position)*fabs(AeYr)/6)/(Time))/0.01;
    gClinet->setMotor_rightElbowPitch(Qe_Ysend,speede_Y);

    double Qw_Zsend=3600;
    int speedw_Z=((fabs(Qw_Zsend-PRH5.position)*fabs(AwZr)/6)/(Time))/0.01;
    gClinet->setMotor_rightElbowRoll(Qw_Zsend,speedw_Z);

    double Qw_Ysend=2241;
    int speedw_Y=((fabs(Qw_Ysend-PRH6.position)*fabs(AwYr)/6)/(Time))/0.01;
    gClinet->setMotor_rightWristPitch(Qw_Ysend,speedw_Y);

    double Qw_Xsend=1566;
    int speedw_X=((fabs(Qw_Xsend-PRH7.position)*fabs(AwXr)/6)/(Time))/0.01;
    gClinet->setMotor_rightWristRoll(Qw_Xsend,speedw_X);
    ...
    //    double Qgrip=2140;
    //    int speed_grip=1000;
    //    gClinet->setMotor_rightGripper(Qgrip,speed_grip);

    //    Qgrip=3200;
    //    speed_grip=1000;
    //    gClinet->setMotor_leftGripper(Qgrip,speed_grip);
    ...
    PW = gClinet->getMotor_waist();
    Qw=2550;
    speedw=((fabs(Qw-PW.position)*fabs(AsYr)/6)/(2*Time))/0.01;
    gClinet->setMotor_waist(Qw,speedw);
    boost::this_thread::sleep(boost::posix_time::milliseconds(2*Time*1000));
...
int Bending()
{
    ...
    double Qs_Ysend=2327;
    int speeds_Y=((fabs(Qs_Ysend-PRH1.position)*fabs(AsYr)/6)/(Time))/0.01;
    gClinet->setMotor_rightShoulderYaw(Qs_Ysend,speeds_Y);

    double Qs_Xsend=3282;
    int speeds_X=((fabs(Qs_Xsend-PRH2.position)*fabs(AsXr)/6)/(Time))/0.01;
    gClinet->setMotor_rightShoulderPitch(Qs_Xsend,speeds_X);
    ...
    double Qw_Zsend=3000;
    int speedw_Z=((fabs(Qw_Zsend-PRH5.position)*fabs(AwZr)/6)/(Time))/0.01;
    gClinet->setMotor_rightElbowRoll(Qw_Zsend,speedw_Z);

    double Qw_Ysend=2370;
    int speedw_Y=((fabs(Qw_Ysend-PRH6.position)*fabs(AwYr)/6)/(Time))/0.01;
    gClinet->setMotor_rightWristPitch(Qw_Ysend,speedw_Y);

    double Qw_Xsend=1582;
    int speedw_X=((fabs(Qw_Xsend-PRH7.position)*fabs(AwXr)/6)/(Time))/0.01;
    gClinet->setMotor_rightWristRoll(Qw_Xsend,speedw_X);
...
int Bye_Bye()
{
    ...
    double Qs_Ysend=2064;
    int speeds_Y=((fabs(Qs_Ysend-PRH1.position)*fabs(AsYr)/6)/(Time))/0.01;
    gClinet->setMotor_rightShoulderYaw(Qs_Ysend,speeds_Y);

    double Qs_Xsend=3369;
    int speeds_X=((fabs(Qs_Xsend-PRH2.position)*fabs(AsXr)/6)/(Time))/0.01;
    gClinet->setMotor_rightShoulderPitch(Qs_Xsend,speeds_X);

    double Qs_Zsend=1810;
    int speeds_Z=((fabs(Qs_Zsend-PRH3.position)*fabs(AsZr)/6)/(Time))/0.01;
    gClinet->setMotor_rightShoulderRoll(Qs_Zsend,speeds_Z);

    double Qe_Ysend=2304;
    int speede_Y=((fabs(Qe_Ysend-PRH4.position)*fabs(AeYr)/6)/(Time))/0.01;
    gClinet->setMotor_rightElbowPitch(Qe_Ysend,speede_Y);

    double Qw_Zsend=1600;
    int speedw_Z=((fabs(Qw_Zsend-PRH5.position)*fabs(AwZr)/6)/(Time))/0.01;
    gClinet->setMotor_rightElbowRoll(Qw_Zsend,speedw_Z);

    double Qw_Ysend=2258;
    int speedw_Y=((fabs(Qw_Ysend-PRH6.position)*fabs(AwYr)/6)/(Time))/0.01;
    gClinet->setMotor_rightWristPitch(Qw_Ysend,speedw_Y);

    double Qw_Xsend=1218;
    int speedw_X=((fabs(Qw_Xsend-PRH7.position)*fabs(AwXr)/6)/(Time))/0.01;
    gClinet->setMotor_rightWristRoll(Qw_Xsend,speedw_X);
    ...
    gClinet->setMotor_rightGripper(3307,200);
    // Right hand //mire aghab
    PRH1 = gClinet->getMotor_rightShoulderYaw();
    PRH2 = gClinet->getMotor_rightShoulderPitch();
    PRH3 = gClinet->getMotor_rightShoulderRoll();
...
int Hand_Shaking()
{
    ...
    double Qs_Ysend=2360;
    int speeds_Y=((fabs(Qs_Ysend-PRH1.position)*fabs(AsYr)/6)/(Time))/0.01;
    gClinet->setMotor_rightShoulderYaw(Qs_Ysend,speeds_Y);

    double Qs_Xsend=3397;
    int speeds_X=((fabs(Qs_Xsend-PRH2.position)*fabs(AsXr)/6)/(Time))/0.01;
    gClinet->setMotor_rightShoulderPitch(Qs_Xsend,speeds_X);

    double Qs_Zsend=1519;
    int speeds_Z=((fabs(Qs_Zsend-PRH3.position)*fabs(AsZr)/6)/(Time))/0.01;
    gClinet->setMotor_rightShoulderRoll(Qs_Zsend,speeds_Z);
    ...
    double Qw_Zsend=2600;
    int speedw_Z=((fabs(Qw_Zsend-PRH5.position)*fabs(AwZr)/6)/(Time))/0.01;
    gClinet->setMotor_rightElbowRoll(Qw_Zsend,speedw_Z);

    double Qw_Ysend=2419;
    int speedw_Y=((fabs(Qw_Ysend-PRH6.position)*fabs(AwYr)/6)/(Time))/0.01;
    gClinet->setMotor_rightWristPitch(Qw_Ysend,speedw_Y);

    double Qw_Xsend=1450;
    int speedw_X=((fabs(Qw_Xsend-PRH7.position)*fabs(AwXr)/6)/(Time))/0.01;
    gClinet->setMotor_rightWristRoll(Qw_Xsend,speedw_X);
    ...
    gClinet->setMotor_rightGripper(2500,1000);
    boost::this_thread::sleep(boost::posix_time::milliseconds(1500));
    double i;
    for (i==0;i<3;i++)
    {
    ...
    boost::this_thread::sleep(boost::posix_time::milliseconds(1000));
    gClinet->setMotor_rightGripper(3307,1000);
    boost::this_thread::sleep(boost::posix_time::milliseconds(1000));
    // Right hand //mire aghab
    PRH1 = gClinet->getMotor_rightShoulderYaw();
    PRH2 = gClinet->getMotor_rightShoulderPitch();
...
string sound_folder = "/home/cast/Desktop/sound_final/";

int show()
{

    sound_client->play(sound_folder + "2-hand.mp3");
    boost::this_thread::sleep(boost::posix_time::milliseconds(10000));
    sound_client->pause();
    ...
    double Qs_Ysend=1617;
    int speeds_Y=((fabs(Qs_Ysend-PLH1.position)*fabs(AsYl)/6)/(Time))/0.01;
    gClinet->setMotor_leftShoulderYaw(Qs_Ysend,speeds_Y);

    double  Qs_Xsend=920;
    int  speeds_X=((fabs(Qs_Xsend-PLH2.position)*fabs(AsXl)/6)/(Time))/0.01;
    gClinet->setMotor_leftShoulderPitch(Qs_Xsend,speeds_X);

    double Qs_Zsend=2414;
    int  speeds_Z=((fabs(Qs_Zsend-PLH3.position)*fabs(AsZl)/6)/(Time))/0.01;
    gClinet->setMotor_leftShoulderRoll(Qs_Zsend,speeds_Z);

    double Qe_Ysend=1969;
    int  speede_Y=((fabs(Qe_Ysend-PLH4.position)*fabs(AeYl)/6)/(Time))/0.01;
    gClinet->setMotor_leftElbowPitch(Qe_Ysend,speede_Y);

    double Qw_Zsend=1953;
    int  speedw_Z=((fabs(Qw_Zsend-PLH5.position)*fabs(AwZl)/6)/(Time))/0.01;
    gClinet->setMotor_leftElbowRoll(Qw_Zsend,speedw_Z);

    double Qw_Ysend=1111;
    int  speedw_Y=((fabs(Qw_Ysend-PLH6.position)*fabs(AwYl)/6)/(Time))/0.01;
    gClinet->setMotor_leftWristPitch(Qw_Ysend,speedw_Y);

    double Qw_Xsend=931;
    int  speedw_X=((fabs(Qw_Xsend-PLH7.position)*fabs(AwXl)/6)/(Time))/0.01;
    gClinet->setMotor_leftWristRoll(Qw_Xsend,speedw_X);
    ...
    speedw_Y=500;
    gClinet->setMotor_leftWristPitch(Qw_Ysend,speedw_Y);
    boost::this_thread::sleep(boost::posix_time::milliseconds(1.3*Time2*1000));
    ...
    speede_Y=500;
    gClinet->setMotor_leftElbowPitch(Qe_Ysend,speede_Y);
    boost::this_thread::sleep(boost::posix_time::milliseconds(0.8*Time2*1000));
    Qe_Ysend=1356;
    speede_Y=500;
    gClinet->setMotor_leftElbowPitch(Qe_Ysend,speede_Y);
    boost::this_thread::sleep(boost::posix_time::milliseconds(Time2*1000));
    ...
    upperbodyclient::motor_data PH1 = gClinet->getMotor_headPitch();
    double Qh1=1157;
    int speedh1=((fabs(Qh1-PH1.position)*fabs(AwXl)/6)/(Time3))/0.01;
    gClinet->setMotor_headPitch(Qh1,speedh1);
    boost::this_thread::sleep(boost::posix_time::milliseconds(Time3*1000));

    PH1 = gClinet->getMotor_headPitch();
    Qh1=1020;
    speedh1=((fabs(Qh1-PH1.position)*fabs(AwXl)/6)/(Time3))/0.01;
    gClinet->setMotor_headPitch(Qh1,speedh1);
    boost::this_thread::sleep(boost::posix_time::milliseconds(Time3*1000));

    upperbodyclient::motor_data PH2 = gClinet->getMotor_headYaw();
    double Qh2=3083;
    int speedh2=((fabs(Qh2-PH2.position)*fabs(AwXl)/6)/(Time3))/0.01;
    gClinet->setMotor_headYaw(Qh2,speedh2);
    boost::this_thread::sleep(boost::posix_time::milliseconds(Time3*1000));

    PH2 = gClinet->getMotor_headYaw();
    Qh2=4027;
    speedh2=((fabs(Qh2-PH2.position)*fabs(AwXl)/6)/(2*Time3))/0.01;
    gClinet->setMotor_headYaw(Qh2,speedh2);
    boost::this_thread::sleep(boost::posix_time::milliseconds(2*Time3*1000));

    PH2 = gClinet->getMotor_headYaw();
    Qh2=3555;
    speedh2=((fabs(Qh2-PH2.position)*fabs(AwXl)/6)/(Time3))/0.01;
    gClinet->setMotor_headYaw(Qh2,speedh2);
    boost::this_thread::sleep(boost::posix_time::milliseconds(Time3*1000));
    ...
    upperbodyclient::motor_data  Pw = gClinet->getMotor_waist();
    double Qw=1830;
    int speedw=((fabs(Qw-Pw.position)*fabs(AwXl)/6)/(Time3))/0.01;
    gClinet->setMotor_waist(Qw,speedw);
    boost::this_thread::sleep(boost::posix_time::milliseconds(Time3*1000));

    Pw = gClinet->getMotor_waist();
    Qw=2550;
    speedw=((fabs(Qw-Pw.position)*fabs(AwXl)/6)/(2*Time3))/0.01;
    gClinet->setMotor_waist(Qw,speedw);
    boost::this_thread::sleep(boost::posix_time::milliseconds(2*Time3*1000));

    Pw = gClinet->getMotor_waist();
    Qw=2190;
    speedw=((fabs(Qw-Pw.position)*fabs(AwXl)/6)/(Time3))/0.01;
    gClinet->setMotor_waist(Qw,speedw);
    boost::this_thread::sleep(boost::posix_time::milliseconds(Time3*1000));
...
void amg_thread()
{
    ...
    while (ros::ok() && appExit == false )
    {
        if ( amg_vector.size() != 0 )
        {
            ...
            if ( command == "Greeting")
            {
                cout<<"GET Greeting "<<endl;
                show();
            }

            if ( command == "Bending")
            {
                cout<<"GET Bending "<<endl;
                Bending();
            }

            if ( command == "Standing")
            {
                cout<<"GET Standing "<<endl;
                Standing();
            }
            ...
            if ( command == "HandShaking")
            {
                cout<<"GET HandShaking"<<endl;
                Hand_Shaking();
            }
