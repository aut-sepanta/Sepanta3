...
void chatterCallback_head(const sepanta_msgs::head::ConstPtr &msg)
{
    g_Motor[9] = msg->pan;
    g_Motor[7] = msg->tilt;
}

void chatterCallback_arm_right(const sepanta_msgs::arm::ConstPtr &msg)
{
...
void chatterCallback_arm_left(const sepanta_msgs::arm::ConstPtr &msg)
{
...
void serial_logic()
{
    ...
    while (App_exit == false)
    {
        try
        {
            try
            {
                serial::Serial my_serial("/dev/serial/by-id/usb-ROBOTIS_CO._LTD._ROBOTIS_Virtual_COM_Port-if00", 115200 , serial::Timeout::simpleTimeout(500));
...
int main(int argc, char **argv)
{
    ...
    chatter_pub[4] = node_handles[2].advertise<sepanta_msgs::omnidata>("AUTROBOTOUT_omnispeed", 10);
    chatter_pub[6] = node_handles[3].advertise<sepanta_msgs::irsensor>("AUTROBOTOUT_irsensor", 10);
    chatter_pub[7] = node_handles[4].advertise<sepanta_msgs::irsensor>("AUTROBOTOUT_lasersensor", 10);
